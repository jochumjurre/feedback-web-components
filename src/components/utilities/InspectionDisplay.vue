        ...
        <v-card v-if="activeFinding" class="mx-auto">
            ...
            <v-card-text class="text-body-1">
                ...
                <p v-show="activeFinding.subType">
                    <span class="font-weight-medium">Sub-type: </span>{{ activeFinding.subType }}
                </p>
                <p v-show="activeFinding.description">
                    <span class="font-weight-medium">Omschrijving: </span>{{ activeFinding.description }}
                </p>
                <p v-show="activeFinding.reportedMalfunctions">
                    <span class="font-weight-medium">Gemelde storingen: </span>{{ activeFinding.reportedMalfunctions }}
                </p>
                ...
                <p v-show="activeFinding.costEstimate">
                    <span class="font-weight-medium">Kostenindicatie: </span>{{ activeFinding.costEstimate }}
                </p>
                <p v-show="activeFinding.performedBy">
                    <span class="font-weight-medium">Uitgevoerd door: </span>{{ activeFinding.performedBy }}
                </p>
                <p v-show="activeFinding.remarks">
                    <span class="font-weight-medium">Opmerkingen: </span>{{ activeFinding.remarks }}
                </p>
                ...
                <p v-show="activeFinding.testProcedurePdf">
                    ...
                    <a :href="getPdfUrl(activeFinding.testProcedurePdf)">{{ activeFinding.testProcedurePdf }}</a>
                </p>
                <p v-show="activeFinding.existingSituationPdf">
                    <span class="font-weight-medium">Testprocedure (.PDF): </span>
                    <a :href="getPdfUrl(activeFinding.existingSituationPdf)">{{ activeFinding.existingSituationPdf }}</a>
                </p>
            </v-card-text>
        </v-card>
    </div>
